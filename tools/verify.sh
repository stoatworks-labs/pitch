...
#                 was fitted to it. Each is a closed form measured out of the
#                 picture:
#                   --identity  neutral settings return the input byte for byte
#                   --energy    the optics conserve light, blurred or not
#                   --moire     the fringe period is 1 / | s - round( s ) | px
#                   --bands     the band period is P / ( Tr / H ) rows and a
#                               whole-period shutter has none
#                   --pwm       band depth against the closed-form overlap
#                   --faults    a dead cabinet is exactly black and aligned
...
if [ -f demo/tools/check_shaders.py ]; then
	if python3 demo/tools/check_shaders.py >/tmp/pitch-demo-shaders.log 2>&1; then
		pass "$( tail -1 /tmp/pitch-demo-shaders.log )"
	else
		fail "the demo's shaders have drifted -- see /tmp/pitch-demo-shaders.log"
		tail -12 /tmp/pitch-demo-shaders.log
	fi
else
...
for size in 320x180 1280x720; do
	...
	for check in identity energy moire bands pwm faults bayer negative; do
		if out=$("$PITEST" --$check --size $size 2>&1); then
			pass "pitest --$check: $( printf '%s\n' "$out" | grep -v '^$' | tail -1 )"
		else
			fail "pitest --$check at $size"
			printf '%s\n' "$out" | sed 's/^/      /'
		fi
	done
done
...
"$PITEST" --bench --frames 60 2>&1 | sed -n '3,7p' | sed 's/^/   /'

BUNDLE="$BUILD/Pitch.bundle"
BIN="$BUNDLE/Contents/MacOS/Pitch"
...
if [ "$(uname)" = "Darwin" ] && [ -d "$BUNDLE" ]; then
	...
	if [ "$ident" = "com.stoatworks.ffgl.pitch" ]; then
	...
	if codesign --force --sign - --timestamp=none "$tmp/Pitch.bundle" >/dev/null 2>&1; then
	...
	if [ -x "$OXBOW" ]; then
		...
		for want in "name:        SW Pitch" "id:          PI01" "type:        effect"; do
			case "$probe" in
