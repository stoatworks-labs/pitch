...
card and reports any that made no difference at all.
    ...
    python3 tools/sweep.py [--binary build/pitest] [--size WxH] [--jobs N]

Exit code 1 means something is dead.

------------------------------------------------------------------ the traps

**A phase of one whole period is a phase of zero.** Refresh Phase runs
0..1 of a refresh period, so its two ends render identically and it is swept
against a half.

**The wall is centred, so Cabinet W moves nothing visible.** A different
cabinet count changes where the wall ends, and the end is outside the frame at
the defaults; the boundaries only show through faults.

**The fault controls are conditional.** Dead, Dim, Dead Row, Bin Shift, Module
Rows and Fault Seed do nothing until Fault Rate is up, because at the default
rate of zero no cabinet draws a fault. Each carries the context it needs in
`CONTEXT`; a parameter missing from that table is swept on the defaults. That
is the table doing its job: a new control has to say what else must be true.

**An option's range is its element count, an integer's is its real range.**
`pitest --list` prints both for exactly this reason, and the ends are what get
swept. Grey Bits 4 against 16 posterises visibly; 8 against 16 would not, on an
8-bit card, and would be reported dead while being correct.
...
FAULTS = {"Fault Rate": 1.0}
CONTEXT = {
    # The wall is centred on the picture, so a different cabinet count moves
    # the wall's edge and nothing else -- and the edge is outside the frame at
    # the defaults. Cabinet boundaries become visible through faults. (Cabinet
    # H happens to sweep live without this, because shifting every LED row's
    # index changes its scan group; that is luck, not coverage.)
    "Cabinet W": FAULTS,
    "Cabinet H": FAULTS,
...
def main():
    ...
    ap.add_argument("--binary", default=str(ROOT / "build" / "pitest"))
    ...
    scratch = tempfile.mkdtemp(prefix="pisweep")
