...
`Controls.cpp`, the per-row exposure windows out of `Pitch::ProcessOpenGL`
(split into whole and fraction, as `split()` does), the wall's cabinet count
and origin, and the frame-period smoothing in plugin.js are a hand translation,
and only a reader can tell whether they still agree. When you change one in
C++, change it there too.
"""
import re
import sys
...
PAIRS = [
    ...
    ("source/Shaders.cpp", "kWallShader", "WALL"),
    ("source/Shaders.cpp", "kSensorShader", "SENSOR"),
    ("source/Shaders.cpp", "kDemosaicShader", "DEMOSAIC"),
]
