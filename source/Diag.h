...
/**
    ...
    **A shader that will not compile.** `InitGL` returns `FF_FAIL` and from
    ...
    message anywhere; with two passes this also records *which* one, which is
    ...
    **A pass buffer that could not be allocated.** This plugin holds up to
    the wall, the sensor and the mip chain of the picture, and the failure mode when the
    driver says no is a black frame with nothing to explain it. What goes in
    the log is the size asked for and how much it came to, because the fix is
    almost always Pitch or Camera Scale -- the buffers are sized from those.
*/
namespace pitch::diag
{
